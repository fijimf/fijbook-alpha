...
import com.fijimf.deepfij.models.{DBOAuth2Info, UserRepository}
...
/**
  ...
  */
class OAuth2InfoDAO @Inject() (protected val dbConfigProvider: DatabaseConfigProvider, protected val userRepo:UserRepository, val actorSystem: ActorSystem)(implicit ec: ExecutionContext)
  ...
  protected def oAuth2InfoQuery(loginInfo: LoginInfo) = for {
    ...
    dbOAuth2Info <- userRepo.oAuth2Infos if dbOAuth2Info.loginInfoId === dbLoginInfo.id
  } yield dbOAuth2Info
  ...
  userRepo.oAuth2Infos.filter(_.loginInfoId in userRepo.loginInfoQuery(loginInfo).map(_.id))
  ...
  protected def addAction(loginInfo: LoginInfo, authInfo: OAuth2Info) =
    userRepo.loginInfoQuery(loginInfo).result.head.flatMap { dbLoginInfo =>
      userRepo.oAuth2Infos += DBOAuth2Info(
        None,
  ...
  def save(loginInfo: LoginInfo, authInfo: OAuth2Info): Future[OAuth2Info] = {
    val query = for {
      result <- userRepo.loginInfoQuery(loginInfo).joinLeft(userRepo.oAuth2Infos).on(_.id === _.loginInfoId)
    } yield result
