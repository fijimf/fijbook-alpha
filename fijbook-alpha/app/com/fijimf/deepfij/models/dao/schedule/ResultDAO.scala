...
trait ResultDAO extends DAOSlick {
  ...
  import scala.concurrent.ExecutionContext.Implicits.global
   ...
   def listResults: Future[List[Result]] = db.run(repo.results.to[List].result)

   def saveResult(result: Result): Future[Result] = saveResults(List(result)).map(_.head)
