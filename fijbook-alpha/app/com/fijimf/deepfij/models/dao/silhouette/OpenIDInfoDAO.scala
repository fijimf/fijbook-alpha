...
/**
  ...
  */
class OpenIDInfoDAO @Inject() (protected val dbConfigProvider: DatabaseConfigProvider, protected val userRepo:UserRepository, val actorSystem: ActorSystem)(implicit ec: ExecutionContext)
  ...
  protected def openIDInfoQuery(loginInfo: LoginInfo) = for {
    ...
    dbOpenIDInfo <- userRepo.openIDInfos if dbOpenIDInfo.loginInfoId === dbLoginInfo.id
  } yield dbOpenIDInfo
  ...
  protected def addAction(loginInfo: LoginInfo, authInfo: OpenIDInfo) =
    userRepo.loginInfoQuery(loginInfo).result.head.flatMap { dbLoginInfo =>
      DBIO.seq(
        userRepo.openIDInfos += DBOpenIDInfo(authInfo.id, dbLoginInfo.id.get),
        userRepo.openIDAttributes ++= authInfo.attributes.map {
  ...
  protected def updateAction(loginInfo: LoginInfo, authInfo: OpenIDInfo) =
    openIDInfoQuery(loginInfo).result.head.flatMap { dbOpenIDInfo =>
      DBIO.seq(
        userRepo.openIDInfos filter(_.id === dbOpenIDInfo.id) update dbOpenIDInfo.copy(id = authInfo.id),
        userRepo.openIDAttributes.filter(_.id === dbOpenIDInfo.id).delete,
        userRepo.openIDAttributes ++= authInfo.attributes.map {
          case (key, value) => DBOpenIDAttribute(authInfo.id, key, value)
        })
    }.transactionally
  ...
  def find(loginInfo: LoginInfo): Future[Option[OpenIDInfo]] = {
    val query = openIDInfoQuery(loginInfo).joinLeft(userRepo.openIDAttributes).on(_.id === _.id)
  ...
  def save(loginInfo: LoginInfo, authInfo: OpenIDInfo): Future[OpenIDInfo] = {
    val query = userRepo.loginInfoQuery(loginInfo).joinLeft(userRepo.openIDInfos).on(_.id === _.loginInfoId)
  ...
  def remove(loginInfo: LoginInfo): Future[Unit] = {
    ...
    val openIDInfoSubQuery = userRepo.openIDInfos.filter(_.loginInfoId in userRepo.loginInfoQuery(loginInfo).map(_.id))
    val attributeSubQuery = userRepo.openIDAttributes.filter(_.id in openIDInfoSubQuery.map(_.id))
