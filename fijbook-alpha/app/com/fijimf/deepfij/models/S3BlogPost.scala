...
case class S3BlogPost(key: String, relativeKey:String, lastModified: LocalDateTime, tags: Map[String, String])

object S3BlogPost {

  val bucket ="fijimf-2017"
  val staticPageFolder="blog/"

  def list(s3: AmazonS3, bucket: String, folder: String): List[S3BlogPost] = {
    s3.listObjects(bucket, folder).getObjectSummaries.map(os => {
      ...
      S3BlogPost(key, key.replace(folder,""), LocalDateTime.ofInstant(Instant.ofEpochMilli(lastModified.getTime), ZoneOffset.UTC), tags)
    }).toList
  }
