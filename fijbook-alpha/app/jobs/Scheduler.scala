...
/**
  * Schedules the jobs.
  */
class Scheduler @Inject() (system: ActorSystem, @Named("auth-token-cleaner") authTokenCleaner: ActorRef, @Named("schedule-updater") scheduleUpdater: ActorRef, @Named("stats-updater") statsUpdater: ActorRef, @Named("memory-monitor") memoryMonitor: ActorRef) {
