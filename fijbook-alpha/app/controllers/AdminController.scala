...
val log=Logger(this.getClass)
  ...
  def readSnapshot(key:String) = silhouette.SecuredAction.async  { implicit rs =>
    ScheduleSerializer.readSchedulesFromS3(key, scheduleDao, repo).map{case(ts,cs,gs)=>Redirect(routes.AdminController.index()).flashing("info"->s"Loaded schedule from snapshot: $ts teams, $cs conferences, $gs games")}
  }
