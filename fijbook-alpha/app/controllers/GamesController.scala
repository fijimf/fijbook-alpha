...
val predictors = List(
  ...
  def games(modelKey:Option[String]) = silhouette.UserAwareAction.async { implicit rs =>
    ...
    teamDao.loadSchedules().map(ss => {
      val sch = ss.find(_.season.dates.contains(today))
  ...
  private def handleDateRequest(today: LocalDate, modelKey:Option[String], rs:UserAwareRequest[DefaultEnv, AnyContent]) = {
