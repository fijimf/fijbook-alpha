...
class ScheduleUpdateServiceImplStressSpec extends PlaySpec with GuiceOneAppPerTest with BeforeAndAfterEach with RebuildDatabaseMixin {
  ...
  def createNTeams(numTeams: Int): List[Team] = {
    val result: List[Team] = Await.result(
      dao.saveTeams(1.to(numTeams).map(n => Team(0L, "team-" + n.toString, "Team-" + n.toString, "A", "a1s", "c1", None, None, None, None, None, None, None, LocalDateTime.now(), "Test")).toList),
      testDbTimeout
    )
  ...
  private def quikGame(season: Season, tup: (Team, Team), gameTime: LocalDateTime): Game = {
  ...
  "Games & results " should {
    ...
    "save games" in new WithApplication() {
      ...
      Await.result(svc.updateDb( keys, gameMapping).andThen {
        ...
        case Failure(ex) => fail("svc.updateDb threw an unexpected exception",ex)
      }, testDbTimeout)
    }
    ...
    "delete games & results" in new WithApplication() {
      ...
      private val keys = gameMapping.map(_.sourceKey).distinct
