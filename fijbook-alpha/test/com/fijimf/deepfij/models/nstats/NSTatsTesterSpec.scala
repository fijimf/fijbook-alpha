    ...
    "load a schedule and loop over it to calculate stats" in {
      Json.parse(s3Sched).asOpt[MappedUniverse] match {
        case Some(uni) =>
          ...
          Analysis.analyzeSchedule(ss(0), Counters.games, println(_))
          Analysis.analyzeSchedule(ss(0), Counters.wins, println(_))
          Analysis.analyzeSchedule(ss(0), Counters.losses, println(_))
          Analysis.analyzeSchedule(ss(0), Counters.otGames, println(_))
          Analysis.analyzeSchedule(ss(0), Counters.homeLosses, println(_))
          Analysis.analyzeSchedule(ss(0), Counters.awayWins, println(_))
          Analysis.analyzeSchedule(ss(0), Counters.otLosses, println(_))
          Analysis.analyzeSchedule(ss(0), Appenders.meanMargin, println(_))
          Analysis.analyzeSchedule(ss(0), Appenders.varianceMargin, println(_))
