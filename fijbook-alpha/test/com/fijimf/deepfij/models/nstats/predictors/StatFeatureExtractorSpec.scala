      ...
      Json.parse(s3Sched).asOpt[MappedUniverse] match {
        case Some(uni) =>
          ...
          ss.foreach(s => {
            ...
            val featureSets: List[Map[String, Double]] = Await.result(extractor(s.games), testDbTimeout)
            assert(featureSets.size === s.games.size)
            featureSets.foreach(fs => {
              assert(fs.size <= 2)
              assert(fs.size < 2 || fs.keySet.contains("wins.value.home"))
              assert(fs.size < 2 || fs.keySet.contains("wins.value.away"))
            })
            assert(featureSets.count(_.size === 2) > 0)
          })
          ...
          ss.foreach(s => {
            ...
            assert(featureSets.size === s.games.size)
            featureSets.foreach(fs => {
              assert(fs.size <= 6, s"Wrong number of keys ${fs.keySet.mkString(", ")}")
              assert(fs.size < 6 || fs.keySet.contains("wins.value.home"))
              assert(fs.size < 6 || fs.keySet.contains("wins.value.away"))
              assert(fs.size < 6 || fs.keySet.contains("wins.rank.home"))
              assert(fs.size < 6 || fs.keySet.contains("wins.rank.away"))
              assert(fs.size < 6 || fs.keySet.contains("mean-margin.zscore.home"))
              assert(fs.size < 6 || fs.keySet.contains("mean-margin.zscore.away"))
            })
            assert(featureSets.count(_.size === 6) > 0)
          })
        case None => fail()
      }
    }
