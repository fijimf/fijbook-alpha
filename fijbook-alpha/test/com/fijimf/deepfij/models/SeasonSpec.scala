...
class SeasonSpec extends FlatSpec {
  ...
  "Season" should "end on Apr 30" in {
  ...
  "Season (non-leap)" should "should have 182 days" in {
    val season = Season(0L, 2018)
    assert(season.dates.size===180)
  }

 "Season (leap)" should "should have 183 days" in {
    val season = Season(0L, 2020)
    assert(season.dates.size===181)
  }

 "Season" should "identify valid dates" in {
    val season = Season(0L, 2018)
