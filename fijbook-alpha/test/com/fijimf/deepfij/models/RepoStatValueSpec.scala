...
class RepoStatValueSpec extends PlaySpec with OneAppPerTest with BeforeAndAfterEach with RebuildDatabaseMixin  with ScalaFutures {
  ...
  "StatValues " should {
    ...
    "be able to save baby statValues" in new WithApplication(FakeApplication()) {
      ...
      Await.result(dao.saveStatValues(15, dates, List("model"), statValues), testDbTimeout)
    ...
    "be able to save large statValues" in new WithApplication(FakeApplication()) {
      ...
      Await.result(dao.saveStatValues(15, nextSixtyDays, List("model"), statValues), testDbTimeout)
    ...
    "be able to save large statValues with a different batch size" in new WithApplication(FakeApplication()) {
      assertStatValuesIsEmpty()
      private val statValues = 1.to(350).flatMap(t => nextSixtyDays.flatMap(d => List("Stat1", "Stat2", "Stat3").map(st => StatValue(0L, "model", st, t.toLong, d, 0.123)))).toList
      Await.result(dao.saveStatValues(5, nextSixtyDays, List("model"), statValues), testDbTimeout)
      assert(Await.result(dao.listStatValues, testDbTimeout).size == 60 * 350 * 3)
    }
    ...
    "be able to update large statValues" in new WithApplication(FakeApplication()) {
      ...
      Await.result(dao.saveStatValues(5, nextSixtyDays, List("model"), statValues1), testDbTimeout)
      ...
      Await.result(dao.saveStatValues(5, nextSixtyDays, List("model"), statValues2), testDbTimeout)
    ...
    "be able to save statValues in a large batch " in new WithApplication(FakeApplication()) {
      assertStatValuesIsEmpty()
      private val statValues = 1.to(350).flatMap(t => nextSixtyDays.flatMap(d => List("Stat1", "Stat2", "Stat3").map(st => StatValue(0L, "model", st, t.toLong, d, 0.123)))).toList
      Await.result(dao.saveStatValues(60, nextSixtyDays, List("model"), statValues), testDbTimeout)
      assert(Await.result(dao.listStatValues, testDbTimeout).size == 60 * 350 * 3)
      private val result = Await.result(dao.loadStatValues("model"), testDbTimeout)
      assert(result.size == 60 * 350 * 3)
    }

    "be able to save statValues in a small batch " in new WithApplication(FakeApplication()) {
      assertStatValuesIsEmpty()
      private val statValues = 1.to(350).flatMap(t => nextSixtyDays.flatMap(d => List("Stat1", "Stat2", "Stat3").map(st => StatValue(0L, "model", st, t.toLong, d, 0.123)))).toList
      Await.result(dao.saveStatValues(1, nextSixtyDays, List("model"), statValues), testDbTimeout)
      assert(Await.result(dao.listStatValues, testDbTimeout).size == 60 * 350 * 3)
      private val result = Await.result(dao.loadStatValues("model"), testDbTimeout)
      assert(result.size == 60 * 350 * 3)
    }
